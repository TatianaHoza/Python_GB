...
Функция deposit(amount) позволяет клиенту пополнять свой счет на определенную сумму. Пополнение счета возможно только на сумму, которая кратна MULTIPLICITY. Если сумма пополнения превышает RICHNESS_SUM, то начисляется налог на богатство в размере RICHNESS_PERCENT процентов.
...
def deposit(amount):
    ...
    print(f'Пополнение карты на {amount} у.е. Итого {my_card} у.е.')
...
def withdraw(amount):
    ...
    if check_multiplicity(amount) == True:
        if amount < 600:
            pr = MIN_REMOVAL
            my_card -= (amount + (amount * pr))
        else:
            pr = MAX_REMOVAL
            my_card -= (amount + (amount * pr))

    print(f'Снятие с карты {check_multiplicity(amount)} у.е. Процент за снятие {pr} . Итого {my_card} у.е.')


def exit():
    global my_card
    if my_card > RICHNESS_SUM:
        my_card -= my_card * RICHNESS_PERCENT
    print(f"Возьмите карту на которой {my_card} у.е")
